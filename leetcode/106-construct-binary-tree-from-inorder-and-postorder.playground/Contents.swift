...
class Solution {
    ...
    func buildTree(_ inorder: [Int], _ postorder: [Int]) -> TreeNode? {
        guard !inorder.isEmpty || !postorder.isEmpty || inorder.count != postorder.count else { return nil }
        
        for (i, val) in inorder.enumerated() {
            inorderMap[val] = i
        }
        rootIndex = postorder.count - 1
        return buildTree(postorder, 0, inorder.count - 1)
    }
    
    private func buildTree(_ postorder: [Int], _ left: Int, _ right: Int) -> TreeNode? {
        if left > right {
            return nil
        }
        
        let rootValue = postorder[rootIndex]
        rootIndex -= 1
        let root = TreeNode(rootValue)
        let mid = inorderMap[rootValue]!
...
if let root = Solution().buildTree(inorder, postorder) {
    print("root = \(root)")
} else {
    print("empty")
}
